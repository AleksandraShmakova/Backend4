...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if ($errors['radio-2']) {
    ...
    $messages[] = '<div class="pas error">Укажите количество конечностей!</div>';
  }
  if ($errors['super']) {
    setcookie('super_error', '', 100000);
    $messages[] = '<div class="pas error">Выберите хотя бы одну суперспособность!</div>';
  }
  if ($errors['bio']) {
  ...
  if ($errors['check-1']) {
    ...
    $messages[] = '<div class="pas error">Вы не дали согласие на обработку данных!</div>';
  }
...
$user = 'u54915';
$pass = '8078372';
  $db = new PDO('mysql:host=localhost;dbname=u54915', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
   try {
